...
function putInfo(info){
    ...
    let score=Math.max(35-25*Math.log10(info.deviation/info.radius+0.0025), 0);
    ctx.fillText(`direction: ${info.winding>0?'clockwise':'anticlockwise'}`, 20, window.innerHeight-120);
    ctx.fillText(`score:     ${score.toFixed(1)}/100`, 20, window.innerHeight-100);
    ctx.fillText(`radius:    ${info.radius.toFixed(1)}px`, 20, window.innerHeight-80);
    ctx.fillText(`deviation: ${info.deviation.toFixed(1)}px`, 20, window.innerHeight-60);
    ctx.fillText(`dipoleErr:${info.bipole.toFixed(1)}px`, 20, window.innerHeight-40);
    ctx.fillText(`developer: Joat917@github`, 20, window.innerHeight-20);
}
